...
class emu():
    ...
    state = dict()
    responseRoots = [
            'NetworkInfo', 'ApsTable', 'Information', 'TimeCluster', 'NwkTable', 'Information',
            'PriceCluster', 'DeviceInfo', 'Google', 'SimpleMeteringCluster', 'InstantaneousDemand',
            'BlockPriceDetail', 'ConnectionStatus', 'BillingPeriodList', 'MessageCluster',
            'FastPollStatus', 'CurrentSummationDelivered', 'NetworkInfo']
    ...
    def set_current_price(self, price, currency, trailing_digits):
        self.command = copy.copy(self.command_root)
    ...
    def create_serial(self):
        try:
            if self.environment == "osx":
                ...
                self.ser = serial.Serial(
                    self.windows_prefix + str(self.port), self.baud_rate, timeout=self.timeout)
    ...
    def serial_thread(self):
        ...
        # this function is the thread which reads from the serial, writes to
        # the serial and calls parsing functions
    ...
    def start_serial(self):
        self.thread_handle = threading.Thread(
            target=self.serial_thread, args=[])
